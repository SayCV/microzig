...
pub fn main() !void {
    ...
    if (parsed_args.generate != .none) {
        ...
        for (region_names, parsed_args.memory_regions) |region_name, region| {
            ...
            try writer.interface.print("  {s} (", .{region_name});

            if (region.access.read) try writer.interface.writeAll("r");
            if (region.access.write) try writer.interface.writeAll("w");
            if (region.access.execute) try writer.interface.writeAll("x");
            ...
            try writer.interface.print(" : ORIGIN = 0x{X:0>8}, LENGTH = 0x{X:0>8}\n", .{ region.offset, region.length });
        }
    ...
    if (parsed_args.generate == .memory_regions_and_sections) {
        ...
        switch (parsed_args.cpu_arch) {
            .arm, .thumb => try writer.interface.print(
                \\
                \\  .ARM.extab : {{
