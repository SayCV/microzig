...
pub fn load_into_db(io: std.Io, db: *Database, path: []const u8) !void {
    ...
    while (try it.next(io)) |entry| {
        ...
        const chip_file_text = try chips_dir.readFileAlloc(io, entry.name, allocator, .limited(std.math.maxInt(usize)));
    ...
    while (try it.next(io)) |entry| {
        ...
        const register_file_text = try registers_dir.readFileAlloc(io, entry.name, allocator, .limited(std.math.maxInt(usize)));
